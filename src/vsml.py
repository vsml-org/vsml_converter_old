...
def element_to_content(
        vsml_element: _Element, style_tree: dict[str, dict[str, str]],
        parent_info_tree: Optional[TagInfoTree] = None, parent_param: Optional[Style] = None,
        count: int = 0,
    ) -> VSMLContent:
    # 子要素の取得
    ...
    # styleの取得
    picked_up_style_tree = pickup_style(style_tree, tag_name, classes_name, id_name, parent_info_tree)
    style = Style(tag_name, parent_param, source_value, picked_up_style_tree, vsml_element.attrib)
    ...
    # vsml_elementがWrapContentの場合
        ...
        for vsml_element_child in vsml_element_children:
            vsml_content.items.append(element_to_content(vsml_element_child, style_tree, tag_info_tree, style, count+1))
    else:
        raise Exception()
