...
VALUE_PATTERN = r"[^;]+"
SELECTORS_PATTERN = rf"{SELECTOR_PATTERN}(\s*(,| )\s*{SELECTOR_PATTERN})*"
PROP_VAL_PATTERN = rf"\s*{PROPERTY_PATTERN}\s*:\s*{VALUE_PATTERN}\s*;?\s*"

ONE_VSS_PATTERN = rf"\s*(?P<selectors>{SELECTORS_PATTERN})\s*\{{(?P<properties>({PROP_VAL_PATTERN})+)\}}\s*"

VSS_PATTERN = rf"^({ONE_VSS_PATTERN})*$"
...
def convert_vss_dict(vss_text: str) -> dict[str, dict[str, str]]:
    ...
    for selector, style in vss_object.items():
        ...
        for property, value in style.items():
            if (
                re.fullmatch(
                    STYLE_VALUE_PATTERN.get(property, ""), value, re.IGNORECASE
                )
                is None
            ):
