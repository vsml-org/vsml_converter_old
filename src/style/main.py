...
class Style:
    ...
    # 各タグのデフォルトparam
    def __init__(
            self, tag_name: str, parent_param: Optional[Style], source_value: str,
            style_tree: dict[str, str], attrib: _Attrib
        ) -> None:
        # inheriting
        if parent_param:
            self.audio_system = parent_param.audio_system if parent_param.audio_system else None
            self.font_color = parent_param.font_color if parent_param.font_color else None
            self.font_border_color = parent_param.font_border_color if parent_param.font_border_color else None
            self.font_border_width = parent_param.font_border_width if parent_param.font_border_width else None
            self.font_family = parent_param.font_family if parent_param.font_family else None
            self.font_size = parent_param.font_size if parent_param.font_size else None
            self.font_weight = parent_param.font_weight if parent_param.font_weight else None
            self.font_style = parent_param.font_style if parent_param.font_style else None
        ...
        match tag_name:
            ...
            case 'vid':
                ...
                if meta_audio is not None:
                    audio_system = AudioSystem.STEREO if meta_audio.get('channel_layout') == 'stereo' else AudioSystem.MONAURAL
                    self.audio_system = self.audio_system if self.audio_system == AudioSystem.MONAURAL else audio_system
                if object_length:
            ...
            case 'txt':
                ...
                self.font_size = self.font_size if self.font_size else GraphicValue("30px")
            case _:
                raise Exception()
        ...
        for param, value in style_tree.items():
            match param:
                ...
                case "margin":
                    ...
                    if parse_value is not None:
                        self.margin_top, self.margin_right, self.margin_bottom, self.margin_left = parse_value
                case "margin-top":
                    parse_value = graphic_parser(value)
                    if parse_value is not None:
                ...
                case "padding":
                    parse_value = multi_graphic_parser(value)
                    if parse_value is not None:
                        self.padding_top, self.padding_right, self.padding_bottom, self.padding_left = parse_value
                case "padding-top":
                    parse_value = graphic_parser(value)
                    if parse_value is not None:
    ...
    def _get_info_from_meta(self, meta: dict) -> tuple[Optional[str], Optional[dict], Optional[dict]]:
...
def pickup_style(
        ...
        class_name: list[str], id_name: Optional[str], parent_info_tree: Optional[TagInfoTree] = None
    ) -> dict[str, str]:
    picked_up_style = {}
